...
全国の駐輪場データ **38,799 件**を同梱し、オフラインでも地図上に表示できます。予約制（akippa・特P 等）の駐輪場では、詳細画面から備考・写真・予約サイトへのリンクも確認できます。

## 主な特徴

- 🗺️ **全国 38,799 件の駐輪場を地図表示** — 時間貸し駐輪場の全国データを同梱
- 📍 **現在地から探せる** — 起動時に現在地へ移動、周辺の最新情報も動的取得
- 🔎 **ライダー向けの絞り込み** — 予約の要否 / 排気量 / 屋根 / 地球ロック / 路面 / 傾斜
- 🅿️ **予約制スポットに対応** — 詳細画面で備考・写真・予約サイトへのリンクを表示
- 📋 **充実した駐輪場情報** — 所在地・TEL・駐車場形態・利用可能時間・料金・収容台数・車両制限・バイク種別・管理会社・最終更新日を表示
- ✏️ **更新・削除依頼 / 情報提供** — 掲載内容の変更依頼や、管理会社からの情報提供の窓口を案内
...
- 公開 Web 版でライブ取得（周辺検索・詳細）を動かすには、データ提供元が CORS 非対応のため
...
lib/
  ...
  services/      駐輪場データ取得・データセット・位置情報・保存リポジトリ・ユーザー設定
  screens/       地図 / 詳細 / 新規登録 / 保存 / 更新・削除依頼 / 管理会社からの情報提供
  widgets/       絞り込みシート 等
assets/
  jmpsa_spots.json   同梱の全国データ(harvest生成)
  icon/              アプリアイコン素材
tool/
  harvest_jmpsa.dart   全国データ一覧の一括取得スクリプト
  enrich_details.dart  全件の詳細項目を取得して焼き込むスクリプト
cloudflare-worker/
...
- 本アプリのデータは提携する公開情報を参照しています。内容の正確性・最新性は保証されません。
